    ...
    public static class ManageScene
    {
        ...
        private static object CaptureScreenshot(SceneCommand cmd)
        {
            try
            {
                ...
                // No camera specified, edit mode, and we have an editor (not batch): grab the GameView
                // window pixels directly. This is the only path that includes Screen Space - Overlay
                // canvases in edit mode — an offscreen Camera render skips them, and the composited
                // ScreenCapture path below requires play mode. GameView grabs are viewport-resolution
                // only, so super_size > 1 requests skip this and use the paths that honor supersizing.
                if (targetCamera == null && resolvedSuperSize <= 1 && !Application.isPlaying && !Application.isBatchMode)
                {
                    try
                    {
                        string gvFolderOverride = ScreenshotPreferences.Resolve(cmd.outputFolder);
                        var gvResult = EditorWindowScreenshotUtility.CaptureGameViewViewportToProject(
                            ...
                            folderOverride: gvFolderOverride);

                        if (ScreenshotUtility.IsUnderAssets(gvResult.ProjectRelativePath))
                            AssetDatabase.ImportAsset(gvResult.ProjectRelativePath, ImportAssetOptions.ForceSynchronousImport);

                        var data = new Dictionary<string, object>
                        {
                            { "path", gvResult.ProjectRelativePath },
                        ...
                        return new SuccessResponse(
                            $"Screenshot captured to '{gvResult.ProjectRelativePath}' (game view, {gvW}x{gvH}).",
                            data);
                    }
                    catch (InvalidOperationException gvEx) when (gvEx.Message.StartsWith("Screenshot folder", StringComparison.Ordinal))
                    {
                        return new ErrorResponse(gvEx.Message);
                    }
